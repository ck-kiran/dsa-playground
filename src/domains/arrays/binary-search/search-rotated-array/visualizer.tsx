interface SearchRotatedArrayVisualizerProps {
  step: any;
}

export function SearchRotatedArrayVisualizer({ step }: SearchRotatedArrayVisualizerProps) {
  const {
    array = [],
    target,
    pointers = {},
  ...
  const { currentValue, result, rotationPoint, sortedHalf, direction } = data;
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="flex gap-2 flex-wrap justify-center max-w-4xl">
        {array.map((value: number, index: number) => {
          ...
          const isFound = action === 'found' && index === result;
          ...
          return (
            <div
              ...
              className={`
                ...
                ${!isHighlighted && !isMid && !isLeft && !isRight && !isFound ? 'border-gray-300 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300' : ''}
              `}
            >
              {value}
              {/* Index label */}
      ...
      {result !== undefined && isComplete && (
        <div className="text-center p-4 border rounded-lg bg-gray-50 dark:bg-gray-800">
          ...
          <div className={`text-2xl font-bold ${result !== -1 ? 'text-green-600' : 'text-red-600'}`}>
            {result !== -1 ? `Found at index ${result}` : 'Not Found (-1)'}
          </div>
        </div>
      )}
