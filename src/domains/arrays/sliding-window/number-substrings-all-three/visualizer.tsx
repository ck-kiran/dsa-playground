interface NumberOfSubstringsContainingAllThreeVisualizerProps {
  step: any;
}

export function NumberOfSubstringsContainingAllThreeVisualizer({ step }: NumberOfSubstringsContainingAllThreeVisualizerProps) {
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      {result !== undefined && (
        <div className="px-3 py-1 bg-green-100 dark:bg-green-900/30 rounded">
          <span className="font-semibold">Count: {result}</span>
        </div>
      )}
      {isComplete && result !== undefined && (
        <div className="text-center p-4 border-2 border-green-500 rounded-lg bg-green-50 dark:bg-green-900/20">
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">Total: {result}</div>
        </div>
      )}
