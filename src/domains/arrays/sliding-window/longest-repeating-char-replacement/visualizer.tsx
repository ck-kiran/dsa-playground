interface LongestRepeatingCharacterReplacementVisualizerProps {
  step: any;
}

export function LongestRepeatingCharacterReplacementVisualizer({ step }: LongestRepeatingCharacterReplacementVisualizerProps) {
          ...
          {text.split('').map((char: string, index: number) => (
            <div
              ...
              className={`w-10 h-10 flex items-center justify-center rounded border-2 text-sm font-semibold transition-all ${
                left === index ? 'border-blue-500 bg-blue-100 dark:bg-blue-900/30 scale-110' :
                right === index ? 'border-orange-500 bg-orange-100 dark:bg-orange-900/30 scale-110' :
      ...
      <div className="flex gap-4">
        {k !== undefined && (
          <div className="px-3 py-1 bg-purple-100 dark:bg-purple-900/30 rounded">
            <span className="font-semibold">Max Replacements: {k}</span>
          </div>
        )}
        {maxLength !== undefined && (
          <div className="px-3 py-1 bg-green-100 dark:bg-green-900/30 rounded">
            <span className="font-semibold">Max Length: {maxLength}</span>
          </div>
        )}
      </div>
