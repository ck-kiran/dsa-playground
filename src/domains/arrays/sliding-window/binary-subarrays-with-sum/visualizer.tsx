interface BinarySubarraysWithSumVisualizerProps {
  step: any;
}

export function BinarySubarraysWithSumVisualizer({ step }: BinarySubarraysWithSumVisualizerProps) {
  ...
  return (
    <div className="flex flex-col items-center space-y-6">
          ...
          {array.map((num: number, index: number) => (
            <div
              ...
              className={`w-10 h-10 flex items-center justify-center rounded border-2 text-sm font-semibold ${
                num === 1 ? 'border-green-500 bg-green-100 dark:bg-green-900/30' : 'border-gray-300 bg-gray-50 dark:bg-gray-800'
              }`}
            >
              {num}
            </div>
      ...
      {goal !== undefined && <div className="px-3 py-1 bg-purple-100 dark:bg-purple-900/30 rounded"><span className="font-semibold">Goal Sum: {goal}</span></div>}
