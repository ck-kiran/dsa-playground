interface SubarraysWithKDifferentIntegersVisualizerProps {
  step: any;
}

export function SubarraysWithKDifferentIntegersVisualizer({ step }: SubarraysWithKDifferentIntegersVisualizerProps) {
  ...
  return (
    <div className="flex flex-col items-center space-y-6">
      <div className="text-center">
        ...
        <div className="flex gap-1 justify-center flex-wrap">
          {array.map((num: number, index: number) => (
            <div
              key={index}
              className="w-10 h-10 flex items-center justify-center rounded border-2 border-gray-300 bg-gray-50 dark:bg-gray-800 text-sm font-semibold"
            >
              {num}
            </div>
          ))}
      ...
      {k !== undefined && <div className="px-3 py-1 bg-purple-100 dark:bg-purple-900/30 rounded"><span className="font-semibold">K = {k} different integers</span></div>}
